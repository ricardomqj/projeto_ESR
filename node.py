...
def handle_connections():
    ...
    while True:
        ...
        print(f"olha a lista {connectionsIP}")
        for val in connectionsIP:
            print(val)
            if val != sender_address:
                forward_ip = val
                print(forward_ip)
                break
        ...
        if forward_ip:
            ...
            connection_socket.sendto(mensagem.encode(), (forward_ip, 9090))
            print(f"Mensagem reencaminhada para {forward_ip}")
        else:
            print("Nenhum IP para reencaminhar a mensagem.")
...
def cliente():
    ...
    try:
        ...
        mesage_thread = threading.Thread(target =handle_connections, args=())
        mesage_thread.start()
